...
Returns detailed moon phase information for the current UTC moment at Greenwich Observatory (51.4779°N, 0.0015°W). This is the moon phase equivalent of `/api/v5/now/subject`.
...
The exact values will vary depending on the current UTC time. The response structure is always the same as `/api/v5/moon-phase`.
...
`sunrise`/`sunset` are Unix timestamps (integers). `sunrise_timestamp`/`sunset_timestamp` are human-readable local time strings (HH:MM).
